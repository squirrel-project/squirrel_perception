...
TEST_F(ClientAttentionBase, testClientAttentionHeight_1) 
{
  ...
  ros::ServiceClient client = n_->serviceClient<squirrel_object_perception_msgs::GetSaliencyHeight>("/squirrel_attention_height");
  squirrel_object_perception_msgs::GetSaliencyHeight srv;
  srv.request.cloud = msg;
  srv.request.height_type.data = 0;
...
{
  n_->getParam ( "ground_truth", ground_truth_);
  ground_truth_ = ground_truth_ + "_2.png";
  readGroundTruth();
  
  ros::ServiceClient client = n_->serviceClient<squirrel_object_perception_msgs::GetSaliencyHeight>("/squirrel_attention_height");
  squirrel_object_perception_msgs::GetSaliencyHeight srv;
  srv.request.cloud = msg;
  srv.request.height_type.data = 2;
...
int main(int argc, char **argv) 
{
  ...
  ros::init(argc, argv, "squirrel_attention_height_test");
