...
TEST_F(ClientAttentionBase, testClientAttentionSymmetry_1) 
{
  n_->getParam ( "ground_truth", ground_truth_);
  readGroundTruth();
  
  ros::ServiceClient client = n_->serviceClient<squirrel_object_perception_msgs::GetSaliencySymmetry>("/squirrel_attention_symmetry");
  squirrel_object_perception_msgs::GetSaliencySymmetry srv;
...
int main(int argc, char **argv) 
{
  ...
  ros::init(argc, argv, "squirrel_attention_symmetry_test");
